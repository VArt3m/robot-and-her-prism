...
- **Mixer** — *targeting; another "sister."* Where a connector relays one colour, a mixer **combines** what it receives into their additive sum and emits that. Two distinct primaries make their secondary (red+green→**yellow**, red+blue→**magenta**, green+blue→**cyan**); a lone secondary passes **straight through** unchanged; a secondary plus a primary already inside it stays that secondary; and any inputs that together cover all three primaries (red+green+blue, red+cyan, yellow+blue, …) sum to **white** — a mixer can *emit* white. It is **confused** by a single base colour (one primary, with nothing to mix) and by **white on its input** — white may be emitted but never received. It is a single device with **no forms**: not programmable, and it **cannot be corrupted** (it always mixes).
...
The **connector** and **inverter** are *programmable* too: at a Forge a relay can be **cleaned** (back to a plain relay) or **corrupted** to a fixed colour — the same locked-colour state a fired rewirer leaves (it then emits that colour regardless of what comes in, never dying on a conflict). The **mixer** is the exception: it is not programmable and cannot be corrupted.
